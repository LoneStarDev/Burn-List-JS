import { Metaplex, Nft, NftError, NftWithToken, Sft, SftWithToken } from "@metaplex-foundation/js";
import { Connection, clusterApiUrl, PublicKey, ParsedAccountData, AccountInfo } from "@solana/web3.js";
...
async function nft_Array_Sort() {
  ...
  }[] = await solana_connection.getParsedProgramAccounts(
    TOKEN_PROGRAM_ID,
    { filters }
  );
  ...
  const account_reducer = (
    ...
    const newMintAddress = (accB.account.data as ParsedAccountData).parsed?.info?.mint;
    if(!newMintAddress) {
      return acc
    }
  ...
  await PromisePool.for(nft_accounts).withConcurrency(10).process(async (mintAddress) => {
